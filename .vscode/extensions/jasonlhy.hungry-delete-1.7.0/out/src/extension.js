...
exports.deactivate = exports.activate = exports.smartBackspace = exports.hungryDelete = exports.setConfig = void 0;
 ...
 */
function findLastLineRange(textDocument, lineNumber, endPosition) {
    const nonEmptyLine = findLastNonEmptyLine(textDocument, lineNumber);
    if (nonEmptyLine) {
        return new vscode_1.Range(nonEmptyLine.range.end, endPosition);
    }
    return new vscode_1.Range(new vscode_1.Position(0, 0), endPosition);
}
/**
 ...
 */
function findDeleteWorldLeftRange(textEditor, textDocument, textLine, endPosition, selection) {
    ...
    if (deleteWhiteSpaceOnly) {
        return new vscode_1.Range(new vscode_1.Position(endPosition.line, lastNonEmptyChar + 1), endPosition);
    }
    // Pretty edge case ....
    ...
    if (configProvider.isStartCoupleCharacters(currentChar)) {
        ...
        return findSmartBackspaceRange(textEditor, textDocument, selection).range;
    }
    if ((lastNonEmptyChar - 1) >= 0) {
        ...
        if (configProvider.isEndCoupleCharacters(currentChar)) {
            if (configProvider.isMatchOpenCoupleCharacters(beforeChar, currentChar) === false) {
                return new vscode_1.Range(findSeparatorStartPosition(text, lastNonEmptyChar, endPosition), endPosition);
            }
        }
    }
    ...
    let wordRange = textDocument.getWordRangeAtPosition(new vscode_1.Position(endPosition.line, lastNonEmptyChar));
    if (wordRange) {
        return new vscode_1.Range(wordRange.start, endPosition);
    }
    return new vscode_1.Range(findSeparatorStartPosition(text, lastNonEmptyChar, endPosition), endPosition);
}
/**
 ...
 * @param position
 */
function findSeparatorStartPosition(text, lastNonEmptyChar, position) {
 ...
 */
function findHungryDeleteRange(textEditor, textDocument, selection) {
...
function findInlineBackspaceRange(position, textDocument) {
    ...
    return (isAutoClosePair) ?
        ...
        new vscode_1.Range(positionBefore, position); // original backspace
}
/**
